...
async function handleLoginPath(req: NextRequest, res: NextResponse) {
  const { encryptedKey, token } = getAuthCookies(req)

  const hasAllCookies = encryptedKey && token
  if (!hasAllCookies) {
    return res
  }
...
async function handleProtectedPath(req: NextRequest, res: NextResponse) {
  const { encryptedKey: cookieEncryptedKey, token: cookieToken } =
    getAuthCookies(req)

  const hasAllCookies = cookieEncryptedKey && cookieToken
  if (!hasAllCookies) {
